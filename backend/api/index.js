...
// ============ GENERATE CODE WITH ROUTE - FIXED ============
app.post('/api/admin/generate-code-with-route/:id', async (req, res) => {
 ...
 try {
   ...
   // 3. CHECK IF EMAIL ALREADY EXISTS (CRITICAL FIX)
   const existingUser = await prisma.user.findUnique({ where: { email: loginEmail } });
   
   if (existingUser && existingUser.id !== id) {
     // Email already taken by another user - append a number
     console.log('⚠️ Email already exists:', loginEmail, 'for user:', existingUser.id);
     
     // Find a unique email by trying numbers
     let counter = 1;
     let uniqueEmail = '';
     let found = false;
     ...
     if (!found) {
       return res.status(500).json({ error: 'Could not generate unique email address' });
     }
     ...
     await prisma.loginCode.deleteMany({ where: { email: uniqueEmail } });
     ...
     const whatsappMessage = `COHT Training Credentials%0A%0A✅ Login Email: ${uniqueEmail}%0A🔑 Code: ${code}%0A%0A🔗 Login: ${BASE_URL}%0A%0A⏰ Code expires in 30 days`;
   ...
   // 4. Email is unique - proceed normally
   const selectedModulesJson = trainingRoute === 'CUSTOMIZED_01' ? JSON.stringify(selectedModules) : null;

...
app.post('/api/admin/generate-code-with-route/:id', async (req, res) => {
 ...
 const BASE_URL = 'https://dsca-mta-quiz.vercel.app';
...
// FIXED SUBMIT ASSESSMENT with proper error tracking using existing 'errors' field
 ...
 questions.forEach((q, i) => { 
   ...
   if (isCorrect) { 
     ...
     const errorDetail = {
       questionNumber: i + 1,
       questionText: q.text,
       userAnswer: q.options ? q.options[userAnswer] : (userAnswer === 0 ? 'True' : (userAnswer === 1 ? 'False' : 'No answer')),

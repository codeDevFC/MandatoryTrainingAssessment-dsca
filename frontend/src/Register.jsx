...
function Register() {
  ...
  const handleReset = () => {
    ...
    setFormData({ firstName: '', lastName: '', email: '', phone: '', role: '', address: '', postCode: '' });
    setSuccess(null);
    setError(null);
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div className="p-6 md:p-8">
              ...
              {!isSubmitted && (
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="+447123456789" required
                      className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition" />
                  ...
                  <div className="flex gap-2 md:gap-3 pt-4">
                    <button type="submit" disabled={loading}
                      ...
                      <Send size={18} /> {loading ? 'Submitting...' : 'SEND'}
                    </button>
                    <button type="button" onClick={handleViewPaymentDetails}
                      ...
                      <CreditCard size={18} /> Click for Payment Details
                    </button>
                  </div>
                </form>
              )}
              ...
              {isSubmitted && (
                <div className="flex gap-2 md:gap-3 pt-4">
                  <button disabled className="flex-1 bg-green-600 text-white py-3 rounded-lg font-semibold flex items-center justify-center gap-2 opacity-75 cursor-not-allowed">
              ...
              {showPayment && (
                <div className="mt-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
                  ...
                  <div className="mt-3 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                    <p className="text-xs text-yellow-800 mb-2">📋 Instructions:</p>
                  ...
                  {hasViewedPayment && (
                    <div className="mt-4 pt-3 border-t border-slate-200">
                      <a href={`https://wa.me/447446180817?text=I%20have%20completed%20payment%20for%20COHT%20Training%20registration.%0A%0AName:%20${encodeURIComponent(formData.firstName + ' ' + formData.lastName)}%0AEmail:%20${encodeURIComponent(formData.email)}%0APhone:%20${encodeURIComponent(formData.phone)}%0A%0APlease%20confirm%20payment%20and%20send%20my%20login%20credentials.`}
                        ...
                        <MessageCircle size={18} /> 📱 Click to Confirm Payment on WhatsApp
                      </a>
